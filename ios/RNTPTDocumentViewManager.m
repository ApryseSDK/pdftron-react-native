...
- (NSDictionary *)addAnnotationToDocument:(nonnull NSNumber *)tag type:(nonnull NSString *)type x1:(nonnull NSNumber *)x1 y1:(nonnull NSNumber *)y1 x2:(nonnull NSNumber *)x2 y2:(nonnull NSNumber *)y2;
{
    RNTPTDocumentView *documentView = self.documentViews[tag];
    if (documentView) {
        return [documentView addAnnotation:type x1:x1 y1:y1 x2:x2 y2:y2];
    } else {
        @throw [NSException exceptionWithName:NSInvalidArgumentException reason:@"Unable to get field for tag" userInfo:nil];
        return nil;
    }
