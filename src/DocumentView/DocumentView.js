...
export default class DocumentView extends PureComponent {
  ...
  exportAsImage = (pageNum) => {
    const tag = findNodeHandle(this._viewerRef);
    if(tag != null) {
      return DocumentViewManager.exportAsImage(tag, pageNum);
    }
    return Promise.resolve();
  }
  ...
  */
  ...
  exportToImage = (pageNumber, dpi, exportFormat) => {
    const tag = findNodeHandle(this._viewerRef);
    if (tag != null) {
       return DocumentViewManager.exportToImage(tag, pageNumber, dpi, exportFormat);
    }
    return Promise.resolve();
  }
