...
export class DocumentView extends PureComponent<DocumentViewProps, any> {
  ...
  addAnnotations = (annotation: AnnotOptions.Annotation): Promise<void> => {
    const tag = findNodeHandle(this._viewerRef);
    if (tag != null) {
      return DocumentViewManager.addAnnotations(tag, annotation);
    }
    return Promise.resolve();
  }
  ...
  addHighlight = (
    ...
    const tag = findNodeHandle(this._viewerRef);
    if (tag != null) {
      return DocumentViewManager.addHighlight(tag, coords, page);
    }
    return Promise.resolve();
  };
