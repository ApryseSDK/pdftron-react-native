...
public class DocumentViewModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    @ReactMethod
    public void addHighlight(final int tag, final ReadableArray coords, final int page, final Promise promise) {
        getReactApplicationContext().runOnUiQueueThread(new Runnable() {
            @Override
            public void run() {
                try {
                    mDocumentViewInstance.addHighlight(tag, coords, page);
                    promise.resolve();
                } catch (Exception ex) {
                    promise.reject(ex);
                }
